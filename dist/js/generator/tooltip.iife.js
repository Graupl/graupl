var TooltipGenerator = (function() {
	...
	var y = class O {
		...
		initialize() {
			try {
				!this._crush && typeof window[this.scope] < "u" && (u(O, { storage: window[this.scope] }, { shouldThrow: !1 }).status || typeof window[this.scope].storage < "u" && typeof window[this.scope].scope < "u" && typeof window[this.scope].type < "u") && (this._storage = window[this.scope].storage);
			} catch {} finally {
 ...
 - `)}`);
		...
		_validate() {
			...
			if (t.status || (this._errors = [...this._errors, ...t.errors], this._valid = !1), Object.keys(this._dom).length > 0) {
				const i = {};
				for (const o of Object.keys(this._dom)) Array.isArray(this._dom[o]) ? this._dom[o].forEach((a, v) => {
					i[`${o}Element[${v}]`] = a;
		...
		_handleKeyup() {}
		_store() {
			this._shouldStore && (u(y, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status || new y({ scope: "GrauplStorage" }), window.GrauplStorage.set({
		...
		_unstore() {
			this._shouldStore && u(y, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status && window.GrauplStorage.clear({
				key: this.id !== "" ? this.id : this.key,
				type: this._storageKey
			});
		}
		...
		_closeOnBlur = !0;
		_name = "Tooltip";
		constructor({ tooltipElement: t, tooltipToggleElement: e, tooltipDescriptionElement: s, showClass: i = "show", hideClass: r = "hide", transitionClass: o = "transitioning", transitionDelay: a = 250, transitionDuration: v = 150, showDuration: F = -1, hideDuration: Q = -1, openOnFocus: P = !1, closeOnBlur: q = !0, hoverType: K = "off", hoverDelay: G = 250, enterDelay: B = -1, leaveDelay: H = -1, prefix: x = "graupl-", key: R = null, initializeClass: U = "initializing", initialize: J = !1 } = {}) {
			super({
				prefix: x,
				key: R,
				initializeClass: U
			}), this._dom.tooltip = t, this._dom.tooltipToggle = e, this._dom.tooltipDescription = s, this._classes.show = i || "", this._classes.hide = r || "", this._classes.transition = o || "", this._durations.transition = v, this._durations.transitionDelay = a, this._durations.show = F, this._durations.hide = Q, this._openOnFocus = P, this._closeOnBlur = q, this._hoverType = K, this._delays.hover = G, this._delays.enter = B, this._delays.leave = H, this._registerEvent("show", { detail: { tooltip: this } }), this._registerEvent("hide", { detail: { tooltip: this } }), this._addEventListener("grauplComponentInitialize", this.rootDOMElement, () => {
				this.hide({ force: !0 });
			}), this._addEventListener("grauplComponentValidate", this.rootDOMElement, () => {
				const C = n("boolean", { isOpen: this._open }, { shouldThrow: !1 });
				C.status || (this._errors = [...this._errors, ...C.errors], this._valid = !1);
				const T = k({ hoverType: this._hoverType }, { shouldThrow: !1 });
		...
		set hoverType(t) {
			k({ hoverType: t }), this._hoverType !== t && (this._hoverType = t);
		}
		get hoverDelay() {
	...
	const D = /[\11\12\14\15\40]+/, g = "data-once", I = document;
	function w(t, e, s) {
		return t[`${e}Attribute`](g, s);
	}
	function p(t) {
		if (typeof t != "string") throw new TypeError("once ID must be a string");
		if (t === "" || D.test(t)) throw new RangeError("once ID must not be empty or contain spaces");
		return `[${g}~="${t}"]`;
	}
	function V(t) {
	...
	function E(t, e, s) {
		return e.filter((i) => {
			const r = V(i) && i.matches(t);
			return r && s && s(i), r;
		});
	}
	function $(t, { add: e, remove: s }) {
		const i = [];
		w(t, "has") && w(t, "get").trim().split(D).forEach((o) => {
	...
	function m(t, e, s) {
		return E(`:not(${p(t)})`, _(e, s), (i) => $(i, { add: t }));
	}
	return m.remove = (t, e, s) => E(p(t), _(e, s), (i) => $(i, { remove: t })), m.filter = (t, e, s) => E(p(t), _(e, s)), m.find = (t, e) => _(t ? p(t) : `[${g}]`, e), ({ options: t = {}, context: e = document, selector: s = ".tooltip", descriptionSelector: i = ".tooltip-description", controllerSelector: r = ".tooltip-toggle" } = {}) => {
