function h(t, e, { shouldThrow: s = !0 } = {}) {
	const i = {
		status: !0,
		errors: []
	};
...
function A(t, e, { shouldThrow: s = !0 } = {}) {
	const i = {
		status: !0,
		errors: []
	};
	try {
		if (n("string", { tagName: t }, { shouldThrow: !0 }).status && h(HTMLElement, e, { shouldThrow: !0 }).status) {
 ...
 - `)}`);
	...
	_validate() {
		...
		if (t.status || (this._errors = [...this._errors, ...t.errors], this._valid = !1), Object.keys(this._dom).length > 0) {
			...
			const r = h(HTMLElement, i, { shouldThrow: !1 });
	...
	_handleKeyup() {}
	_store() {
		this._shouldStore && (h(E, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status || new E({ scope: "GrauplStorage" }), window.GrauplStorage.set({
	...
	_unstore() {
		this._shouldStore && h(E, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status && window.GrauplStorage.clear({
			key: this.id !== "" ? this.id : this.key,
			type: this._storageKey
		});
	}
	...
	_dispatchEvent(t, e) {
		N(t, this), h(HTMLElement, { element: e }), e.dispatchEvent(this.events[t]);
	}
	...
	_shouldStore = !1;
	constructor({ accordionItemElement: t, accordionItemToggleElement: e, accordionItemHeaderElement: s, accordionItemContentElement: i, parentAccordion: r = null }) {
		super({
			prefix: r.prefix,
			key: r.key
		}), this._dom.item = t, this._dom.toggle = e, this._dom.header = s, this._dom.content = i, this._elements.parent = r, this._registerEvent("expand", { detail: { item: this } }), this._registerEvent("collapse", { detail: { item: this } }), this._addEventListener("grauplComponentInitialize", this.rootDOMElement, () => {
	...
	_setAriaAttributes() {
		A("button", { toggle: this.dom.toggle }, { shouldThrow: !1 }).status || this.dom.toggle.setAttribute("role", "button"), this.dom.toggle.getAttribute("aria-expanded") !== "true" && this.dom.toggle.setAttribute("aria-expanded", "false"), this.dom.toggle.setAttribute("aria-controls", this.dom.content.id), A("section", { content: this.dom.content }, { shouldThrow: !1 }).status || this.dom.content.setAttribute("role", "region"), this.dom.content.setAttribute("aria-labelledby", this.dom.toggle.id);
	}
	_reveal({ emit: t = !0, transition: e = !0 } = {}) {
		const { closeClass: s, openClass: i, transitionClass: r, openDuration: o } = this.elements.parent;
		this.dom.toggle.setAttribute("aria-expanded", "true"), e && r !== "" ? (c(r, this.dom.item), requestAnimationFrame(() => {
			d(s, this.dom.item), this.dom.item.style.height = `${this.dom.header.getBoundingClientRect().height}px`, requestAnimationFrame(() => {
				c(i, this.dom.item), this.dom.item.style.height = `${this.dom.header.getBoundingClientRect().height + this.dom.content.getBoundingClientRect().height}px`, requestAnimationFrame(() => {
					this._setTimeout(() => {
						d(r, this.dom.item), this.dom.item.style.height = "";
					}, o);
				});
			});
		})) : (c(i, this.dom.item), d(s, this.dom.item)), this.dom.content.removeAttribute("inert"), t && this._dispatchEvent("expand", this.dom.item);
	}
	_conceal({ emit: t = !0, transition: e = !0 } = {}) {
		const { closeClass: s, openClass: i, transitionClass: r, closeDuration: o } = this.elements.parent;
		this.dom.toggle.setAttribute("aria-expanded", "false"), e && r !== "" ? (c(r, this.dom.item), this.dom.item.style.height = `${this.dom.item.getBoundingClientRect().height}px`, requestAnimationFrame(() => {
			d(i, this.dom.item), this.dom.item.style.height = `${this.dom.header.getBoundingClientRect().height}px`, requestAnimationFrame(() => {
				c(s, this.dom.item), requestAnimationFrame(() => {
					this._setTimeout(() => {
						d(r, this.dom.item), this.dom.item.style.height = "";
					}, o);
				});
			});
		})) : (c(s, this.dom.item), d(i, this.dom.item)), this.dom.content.setAttribute("inert", "true"), t && this._dispatchEvent("collapse", this.dom.item);
	}
	...
	_handleKeydown() {
		this.dom.accordionItemToggles.forEach((t) => {
			this._addEventListener("keydown", t, (e) => {
				const s = f(e);
				["Space", "Enter"].includes(s) ? (l(e), this.currentEvent = "keyboard") : this.optionalKeySupport && [
					"ArrowDown",
					"ArrowUp",
					"Home",
					"End"
				].includes(s) && (l(e), this.currentEvent = "keyboard");
			});
		}), this.dom.accordionControls.forEach((t) => {
			this._addEventListener("keydown", t, (e) => {
				const s = f(e);
				["Space", "Enter"].includes(s) && (l(e), this.currentEvent = "keyboard");
			});
		});
	}
	_handleKeyup() {
		this.dom.accordionItemToggles.forEach((t) => {
			this._addEventListener("keyup", t, (e) => {
				...
				if (this.optionalKeySupport) switch (s) {
					case "Home":
						l(e), this.focusFirstChild();
						break;
					case "End":
						l(e), this.focusLastChild();
						break;
					case "ArrowDown":
			...
			this._addEventListener("keyup", t, (e) => {
				switch (f(e)) {
					case "Space":
					case "Enter": l(e), this.currentEvent = "keyboard", this.allowExpandMultiple && this.openChildren();
				}
			});
		}), this.dom.collapseController.forEach((t) => {
			this._addEventListener("keyup", t, (e) => {
				switch (f(e)) {
					case "Space":
					case "Enter": l(e), this.currentEvent = "keyboard", this.allowCollapseAll && this.closeChildren();
				}
			});
		});
	}
...
const O = /[\11\12\14\15\40]+/, w = "data-once", X = document;
function C(t, e, s) {
	return t[`${e}Attribute`](w, s);
}
function p(t) {
	if (typeof t != "string") throw new TypeError("once ID must be a string");
	if (t === "" || O.test(t)) throw new RangeError("once ID must not be empty or contain spaces");
	return `[${w}~="${t}"]`;
}
function Y(t) {
...
function b(t, e, s) {
	return e.filter((i) => {
		const r = Y(i) && i.matches(t);
		return r && s && s(i), r;
	});
}
function D(t, { add: e, remove: s }) {
	const i = [];
	C(t, "has") && C(t, "get").trim().split(O).forEach((o) => {
...
function g(t, e, s) {
	return b(`:not(${p(t)})`, _(e, s), (i) => D(i, { add: t }));
}
g.remove = (t, e, s) => b(p(t), _(e, s), (i) => D(i, { remove: t })), g.filter = (t, e, s) => b(p(t), _(e, s)), g.find = (t, e) => _(t ? p(t) : `[${w}]`, e);
const tt = ({ options: t = {}, context: e = document, selector: s = ".accordion" } = {}) => {
