...
var E = class {
 ...
 - `)}`);
	...
	_handleKeyup() {}
	_store() {
		this._shouldStore && (h(f, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status || new f({ scope: "GrauplStorage" }), window.GrauplStorage.set({
	...
	_unstore() {
		this._shouldStore && h(f, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status && window.GrauplStorage.clear({
			key: this.id !== "" ? this.id : this.key,
			type: this._storageKey
		});
	}
