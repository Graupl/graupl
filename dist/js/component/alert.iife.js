var Alert = (function() {
 ...
 - `)}`);
		...
		_handleKeyup() {}
		_store() {
			this._shouldStore && (u(d, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status || new d({ scope: "GrauplStorage" }), window.GrauplStorage.set({
		...
		_unstore() {
			this._shouldStore && u(d, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status && window.GrauplStorage.clear({
				key: this.id !== "" ? this.id : this.key,
				type: this._storageKey
			});
		}
		...
		_dispatchEvent(t, e) {
			E(t, this), u(HTMLElement, { element: e }), e.dispatchEvent(this.events[t]);
		}
		...
		_storageKey = "alerts";
		_name = "Alert";
		constructor({ alertElement: t, controllerElement: e = null, showClass: r = "show", hideClass: s = "hide", transitionClass: i = "transitioning", transitionDuration: o = 150, showDuration: a = -1, hideDuration: _ = -1, isHidden: O = !1, prefix: D = "graupl-", key: M = null, initializeClass: S = "initializing", initialize: j = !1 } = {}) {
			super({
				prefix: D,
				key: M,
				initializeClass: S
			}), this._dom.alert = t, this._dom.controller = e, this._classes.show = r || "", this._classes.hide = s || "", this._classes.transition = i || "", this._durations.transition = o, this._durations.show = a, this._durations.hide = _, this._hidden.value = O, this._hidden.commit(), this._registerEvent("show", { detail: { alert: this } }), this._registerEvent("hide", { detail: { alert: this } }), this._addEventListener("grauplComponentInitialize", this.rootDOMElement, () => {
