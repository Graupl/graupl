var AlertGenerator = (function() {
	...
	var j = class {
		...
		initialize() {
			try {
				!this._crush && typeof window[this.scope] < "u" && (u(k, { storage: window[this.scope] }, { shouldThrow: !1 }).status || typeof window[this.scope].storage < "u" && typeof window[this.scope].scope < "u" && typeof window[this.scope].type < "u") && (this._storage = window[this.scope].storage);
			} catch {} finally {
 ...
 - `)}`);
		...
		_handleKeyup() {}
		_store() {
			this._shouldStore && (u(m, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status || new m({ scope: "GrauplStorage" }), window.GrauplStorage.set({
		...
		_unstore() {
			this._shouldStore && u(m, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status && window.GrauplStorage.clear({
				key: this.id !== "" ? this.id : this.key,
				type: this._storageKey
			});
		}
		...
		_dispatchEvent(t, e) {
			S(t, this), u(HTMLElement, { element: e }), e.dispatchEvent(this.events[t]);
		}
		...
		_storageKey = "alerts";
		_name = "Alert";
		constructor({ alertElement: t, controllerElement: e = null, showClass: r = "show", hideClass: s = "hide", transitionClass: i = "transitioning", transitionDuration: o = 150, showDuration: a = -1, hideDuration: w = -1, isHidden: V = !1, prefix: I = "graupl-", key: q = null, initializeClass: P = "initializing", initialize: K = !1 } = {}) {
			super({
				prefix: I,
				key: q,
				initializeClass: P
			}), this._dom.alert = t, this._dom.controller = e, this._classes.show = r || "", this._classes.hide = s || "", this._classes.transition = i || "", this._durations.transition = o, this._durations.show = a, this._durations.hide = w, this._hidden.value = V, this._hidden.commit(), this._registerEvent("show", { detail: { alert: this } }), this._registerEvent("hide", { detail: { alert: this } }), this._addEventListener("grauplComponentInitialize", this.rootDOMElement, () => {
				this.isHidden && this._conceal({
					emit: !1,
					transition: !1
				});
			}), this._addEventListener("grauplComponentValidate", this.rootDOMElement, () => {
				const $ = n("boolean", { isHidden: this._hidden.value }, { shouldThrow: !1 });
	...
	const b = /[\11\12\14\15\40]+/, p = "data-once", L = document;
	function g(t, e, r) {
		return t[`${e}Attribute`](p, r);
	}
	function d(t) {
	...
	function C(t, { add: e, remove: r }) {
		const s = [];
		g(t, "has") && g(t, "get").trim().split(b).forEach((o) => {
	...
	return _.remove = (t, e, r) => y(d(t), f(e, r), (s) => C(s, { remove: t })), _.filter = (t, e, r) => y(d(t), f(e, r)), _.find = (t, e) => f(t ? d(t) : `[${p}]`, e), ({ options: t = {}, context: e = document, selector: r = ".alert" } = {}) => {
