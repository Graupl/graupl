var Disclosure = (function() {
	...
	var y = class {
		...
		initialize() {
			try {
				!this._crush && typeof window[this.scope] < "u" && (c(C, { storage: window[this.scope] }, { shouldThrow: !1 }).status || typeof window[this.scope].storage < "u" && typeof window[this.scope].scope < "u" && typeof window[this.scope].type < "u") && (this._storage = window[this.scope].storage);
			} catch {} finally {
 ...
 - `)}`);
		...
		set focusState(t) {
			v({ focusState: t }), this._focusState !== t && (this._focusState = t);
		}
		get currentEvent() {
		...
		_dispatchEvent(t, e) {
			I(t, this), c(HTMLElement, { element: e }), e.dispatchEvent(this.events[t]);
		}
		...
		constructor({ disclosureElement: t, controllerElement: e, disclosureContentSelector: i = ".disclosure-content", lockedClass: s = "locked", unlockedClass: r = "unlocked", openClass: o = "show", closeClass: a = "hide", transitionClass: m = "transitioning", transitionDuration: L = 250, openDuration: T = -1, closeDuration: z = -1, openOnFocus: A = !1, closeOnBlur: j = !1, minWidth: E = "", breakpoint: p = "", autoOpen: k = !1, openInsideBreakpoint: Q = !1, openOutsideBreakpoint: w = !1, closeInsideBreakpoint: O = !1, closeOutsideBreakpoint: F = !1, lockInsideBreakpoint: V = !1, lockOutsideBreakpoint: P = !1, unlockInsideBreakpoint: x = !1, unlockOutsideBreakpoint: q = !1, locked: K = !1, mediaQuery: G = "", prefix: H = "graupl-", key: R = null, initializeClass: U = "initializing", initialize: J = !1 } = {}) {
			super({
				prefix: H,
				key: R,
				initializeClass: U
			}), this._dom.disclosure = t, this._dom.controller = e, this._selectors.content = i, this._classes.locked = s || "", this._classes.unlocked = r || "", this._classes.open = o || "", this._classes.close = a || "", this._classes.transition = m || "", this._durations.transition = L, this._durations.open = T, this._durations.close = z, this._openOnFocus = A, this._closeOnBlur = j, E !== "" && (console.warn("`minWidth` is deprecated and will be removed in a future release. Please set `breakpoint` instead."), p === "" && (p = E)), k && p !== "" && (console.warn("`autoOpen` is deprecated and will be removed in a future release. Please set `openOutsideBreakpoint` and `closeInsideBreakpoint` to `true` instead."), w = k, O = k), this._breakpoint = p || "", this._openInsideBreakpoint = Q, this._openOutsideBreakpoint = w, this._closeInsideBreakpoint = O, this._closeOutsideBreakpoint = F, this._lockInsideBreakpoint = V, this._lockOutsideBreakpoint = P, this._unlockInsideBreakpoint = x, this._unlockOutsideBreakpoint = q, this._mediaQueryString = G || "", this._locked.value = K, this._locked.commit(), this._registerEvent("expand", { detail: { disclosure: this } }), this._registerEvent("collapse", { detail: { disclosure: this } }), this._registerEvent("lock", { detail: { disclosure: this } }), this._registerEvent("unlock", { detail: { disclosure: this } }), this._addEventListener("grauplComponentInitialize", this.rootDOMElement, () => {
		...
		_setAriaAttributes() {
			this.dom.controller.getAttribute("aria-expanded") !== "true" && this.dom.controller.setAttribute("aria-expanded", "false"), this.dom.controller.setAttribute("aria-controls", this.dom.disclosure.id), $("button", { controller: this.dom.controller }, { shouldThrow: !1 }).status || this.dom.controller.setAttribute("role", "button");
		}
		_setCustomProps() {
...
//# sourceMappingURL=disclosure.iife.js.map