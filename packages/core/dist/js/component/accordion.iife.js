var Accordion = (function() {
 ...
 - `)}`);
		...
		_handleKeyup() {}
		_store() {
			this._shouldStore && (h(p, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status || new p({ scope: "GrauplStorage" }), window.GrauplStorage.set({
		...
		_unstore() {
			this._shouldStore && h(p, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status && window.GrauplStorage.clear({
				key: this.id !== "" ? this.id : this.key,
				type: this._storageKey
			});
		}
