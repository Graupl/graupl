...
var v = class {
 ...
 - `)}`);
	...
	_handleKeyup() {}
	_store() {
		this._shouldStore && (f(m, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status || new m({ scope: "GrauplStorage" }), window.GrauplStorage.set({
	...
	_unstore() {
		this._shouldStore && f(m, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status && window.GrauplStorage.clear({
			key: this.id !== "" ? this.id : this.key,
			type: this._storageKey
		});
	}
	...
	constructor({ shelfElement: t, controllerElement: e, lockControllerElement: s, hoverControllerElement: i, sideControllerElement: r, dependentSelector: o = ".shelf-aware", lockedClass: c = "locked", unlockedClass: _ = "unlocked", hoverClass: E = "hoverable", noHoverClass: C = "not-hoverable", leftClass: b = "left-side", rightClass: w = "right-side", openClass: S = "show", closeClass: L = "hide", transitionClass: $ = "transitioning", transitionDuration: D = 250, openDuration: O = -1, closeDuration: A = -1, hoverable: M = !1, hoverDelay: T = 250, enterDelay: j = -1, leaveDelay: z = -1, locked: H = !1, side: Q = "left", minWidth: V = "1023px", mediaQuery: x = "", prefix: q = "graupl-", key: F = null, initializeClass: I = "initializing", initialize: K = !1 } = {}) {
		super({
			prefix: q,
			key: F,
			initializeClass: I
		}), this._dom.shelf = t, this._dom.controller = e || null, this._dom.lockController = s || null, this._dom.hoverController = i || null, this._dom.sideController = r || null, this._dom.dependents = [], this._selectors.dependents = o, this._classes.locked = c || "", this._classes.unlocked = _ || "", this._classes.hover = E || "", this._classes.noHover = C || "", this._classes.left = b || "", this._classes.right = w || "", this._classes.open = S || "", this._classes.close = L || "", this._classes.transition = $ || "", this._durations.transition = D, this._durations.open = O, this._durations.close = A, this._locked = new v(H), this._breakpoint = V, this._mediaQueryString = x || "", this._side = Q, this._side === "right" && (this._otherSide = "left"), this._hover = M, this._delays.hover = T, this._delays.enter = j, this._delays.leave = z, this._registerEvent("expand", { detail: { shelf: this } }), this._registerEvent("collapse", { detail: { shelf: this } }), this._registerEvent("lock", { detail: { shelf: this } }), this._registerEvent("unlock", { detail: { shelf: this } }), this._registerEvent("shiftSide", { detail: { shelf: this } }), this._registerEvent("enableHover", { detail: { shelf: this } }), this._registerEvent("disableHover", { detail: { shelf: this } }), this._addEventListener("grauplComponentInitialize", this.rootDOMElement, () => {
...
//# sourceMappingURL=navigation-shelf.es.js.map