var NavigationShelf = (function() {
	...
	var g = class {
		...
		initialize() {
			try {
				!this._crush && typeof window[this.scope] < "u" && (f(k, { storage: window[this.scope] }, { shouldThrow: !1 }).status || typeof window[this.scope].storage < "u" && typeof window[this.scope].scope < "u" && typeof window[this.scope].type < "u") && (this._storage = window[this.scope].storage);
			} catch {} finally {
 ...
 - `)}`);
		...
		set focusState(t) {
			E({ focusState: t }), this._focusState !== t && (this._focusState = t);
		}
		get currentEvent() {
		...
		_validate() {
			...
			if (t.status || (this._errors = [...this._errors, ...t.errors], this._valid = !1), Object.keys(this._dom).length > 0) {
				const i = {};
				for (const o of Object.keys(this._dom)) Array.isArray(this._dom[o]) ? this._dom[o].forEach((c, m) => {
					i[`${o}Element[${m}]`] = c;
		...
		_handleKeyup() {}
		_store() {
			this._shouldStore && (f(_, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status || new _({ scope: "GrauplStorage" }), window.GrauplStorage.set({
		...
		_unstore() {
			this._shouldStore && f(_, { storage: window.GrauplStorage }, { shouldThrow: !1 }).status && window.GrauplStorage.clear({
				key: this.id !== "" ? this.id : this.key,
				type: this._storageKey
			});
		}
		...
		_dispatchEvent(t, e) {
			b(t, this), f(HTMLElement, { element: e }), e.dispatchEvent(this.events[t]);
		}
		...
		_removeEventListener(t, e, s, i = {}) {
			...
			this._listeners.forEach((o, c) => {
				o.type === t && o.element === e && o.listener === s && JSON.stringify(o.options) === JSON.stringify(i) && (r = c);
		...
		constructor({ shelfElement: t, controllerElement: e, lockControllerElement: s, hoverControllerElement: i, sideControllerElement: r, dependentSelector: o = ".shelf-aware", lockedClass: c = "locked", unlockedClass: m = "unlocked", hoverClass: A = "hoverable", noHoverClass: M = "not-hoverable", leftClass: T = "left-side", rightClass: j = "right-side", openClass: z = "show", closeClass: H = "hide", transitionClass: Q = "transitioning", transitionDuration: V = 250, openDuration: x = -1, closeDuration: q = -1, hoverable: F = !1, hoverDelay: I = 250, enterDelay: K = -1, leaveDelay: P = -1, locked: G = !1, side: R = "left", minWidth: U = "1023px", mediaQuery: B = "", prefix: N = "graupl-", key: J = null, initializeClass: Z = "initializing", initialize: W = !1 } = {}) {
			super({
				prefix: N,
				key: J,
				initializeClass: Z
			}), this._dom.shelf = t, this._dom.controller = e || null, this._dom.lockController = s || null, this._dom.hoverController = i || null, this._dom.sideController = r || null, this._dom.dependents = [], this._selectors.dependents = o, this._classes.locked = c || "", this._classes.unlocked = m || "", this._classes.hover = A || "", this._classes.noHover = M || "", this._classes.left = T || "", this._classes.right = j || "", this._classes.open = z || "", this._classes.close = H || "", this._classes.transition = Q || "", this._durations.transition = V, this._durations.open = x, this._durations.close = q, this._locked = new g(G), this._breakpoint = U, this._mediaQueryString = B || "", this._side = R, this._side === "right" && (this._otherSide = "left"), this._hover = F, this._delays.hover = I, this._delays.enter = K, this._delays.leave = P, this._registerEvent("expand", { detail: { shelf: this } }), this._registerEvent("collapse", { detail: { shelf: this } }), this._registerEvent("lock", { detail: { shelf: this } }), this._registerEvent("unlock", { detail: { shelf: this } }), this._registerEvent("shiftSide", { detail: { shelf: this } }), this._registerEvent("enableHover", { detail: { shelf: this } }), this._registerEvent("disableHover", { detail: { shelf: this } }), this._addEventListener("grauplComponentInitialize", this.rootDOMElement, () => {
				...
				const v = S({ side: this._side }, { shouldThrow: !1 });
		...
		_handleClick() {
			...
			this.dom.controller && this.dom.controller.addEventListener("click", (t) => {
				t.button === 0 && (this.currentEvent = "mouse", d(t), this.toggle({ preserveLock: !1 }), this.isOpen && (this.focusState = "self", this.isSoftLocked = !0));
			}), this.dom.hoverController && this.dom.hoverController.addEventListener("click", (t) => {
				t.button === 0 && (this.currentEvent = "mouse", d(t), this.focusState = "self", this.toggleHover(), this.hover && this.open());
			}), this.dom.lockController && this.dom.lockController.addEventListener("click", (t) => {
				t.button === 0 && (this.currentEvent = "mouse", d(t), this.focusState = "self", this.toggleLock());
			}), this.dom.sideController && this.dom.sideController.addEventListener("click", (t) => {
				t.button === 0 && (this.currentEvent = "mouse", d(t), this.focusState = "self", this.toggleSide());
